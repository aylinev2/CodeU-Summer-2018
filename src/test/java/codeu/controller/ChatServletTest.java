...
public class ChatServletTest {
  ...
  public void testDoPost_ParsesBBCode() throws IOException, ServletException {
    ...
    Mockito.when(mockRequest.getParameter("message"))
        .thenReturn("Contains [b]html[/b], [s]strikethrough text[/s], and <script>JavaScript</script> content.");
    ...
    Mockito.verify(mockMessageStore).addMessage(messageArgumentCaptor.capture());
    Assert.assertEquals(
        "Contains <b>html</b>, <s>strikethrough text</s>, and  content.", messageArgumentCaptor.getValue().getContent());
  ...
  public void testDoPost_ParsesEmoji() throws IOException, ServletException {
    ...
    Mockito.when(mockRequest.getParameter("message"))
        .thenReturn(":dog: barks and :cat: meows");
    ...
    Mockito.verify(mockMessageStore).addMessage(messageArgumentCaptor.capture());
    Assert.assertEquals(
        "🐶 barks and 🐱 meows", messageArgumentCaptor.getValue().getContent());

    Mockito.verify(mockResponse).sendRedirect("/chat/test_conversation");
  }

  @Test
  public void testDoPost_NullMessage() throws IOException, ServletException {
    Mockito.when(mockRequest.getRequestURI()).thenReturn("/chat/test_conversation");
    Mockito.when(mockSession.getAttribute("user")).thenReturn("test_username");
    ...
    ArgumentCaptor<Message> messageArgumentCaptor = ArgumentCaptor.forClass(Message.class);
    Mockito.verify(mockMessageStore, Mockito.never()).addMessage(messageArgumentCaptor.capture());
    Mockito.verify(mockResponse).sendRedirect("/chat/test_conversation");
  }

  @Test
  public void testDoPost_EmptyMessage() throws IOException, ServletException {
    Mockito.when(mockRequest.getRequestURI()).thenReturn("/chat/test_conversation");
    Mockito.when(mockSession.getAttribute("user")).thenReturn("test_username");
  ...
  @Test
  public void testDoPost_WhitespaceOnlyMessage() throws IOException, ServletException {
    Mockito.when(mockRequest.getRequestURI()).thenReturn("/chat/test_conversation");
    Mockito.when(mockSession.getAttribute("user")).thenReturn("test_username");
